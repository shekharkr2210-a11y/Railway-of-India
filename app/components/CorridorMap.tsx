    ...
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl mb-6 shadow-xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="text-slate-400 font-medium">Filter Dept:</span>
          <button
      ...
      <div className="space-y-6">
        ...
        <div className="relative pt-8 pb-4 px-4 bg-slate-950/70 border border-slate-800/80 rounded-xl overflow-x-auto">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col items-center">
                      <div className="w-5 h-5 rounded-full bg-slate-900 border-2 border-amber-400 flex items-center justify-center shadow-md shadow-amber-500/20 group-hover:scale-125 transition-transform">
                        <div className="w-2 h-2 rounded-full bg-amber-400"></div>
                      </div>
                      <span className="mt-2 text-[10px] font-bold text-slate-300 tracking-wider whitespace-nowrap bg-slate-900/90 px-1.5 py-0.5 rounded border border-slate-800">
                    ...
                    >
                      <div className="flex items-center justify-between text-xs mb-1">
                        <span className="font-bold text-slate-200">{section.code}</span>
                      ...
                      {shadowCount > 0 && (
                        <div className="mt-2 text-[10px] font-semibold text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded-md flex items-center justify-between">
                          ...
                          <span className="text-slate-400 font-normal">Merged</span>
                        </div>
                      )}
                    </div>
                    ...
                    {idx === sections.length - 1 && (
                      <div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 flex flex-col items-center">
                        <div className="w-5 h-5 rounded-full bg-slate-900 border-2 border-emerald-400 flex items-center justify-center shadow-md shadow-emerald-500/20">
        ...
        {selectedSectionId && (
          <div className="bg-slate-950/90 border border-amber-500/30 rounded-xl p-5 animation-fade-in">
            {(() => {
                    ...
                    <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-800">
                      ...
                      <div className="space-y-2">
                        {secTasks.map(task => (
                          <div
                            key={task.id}
                            className="p-2.5 rounded-lg bg-slate-950 border border-slate-800 text-xs flex items-start justify-between"
                          >
                            <div>
                    ...
                    <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-800">
                      ...
                      <div className="space-y-2">
                        {secBlocks.map(block => (
                          <div
                            key={block.id}
                            className="p-2.5 rounded-lg bg-slate-950 border border-amber-500/30 text-xs"
                          >
